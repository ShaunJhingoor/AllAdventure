...
function SearchIndexItem({ result }) {
  ...
  const favoriteForTrail = Object.values(favorites)?.find(
    (favoriteObj) => favoriteObj?.favorite?.trail?.id == result.id
  );
  ...
  const handleFavoriteClick = async () => {
    if (isFavorite) {
      await dispatch(
        removeFromFavorites(result?.id, favoriteForTrail?.favorite?.id)
      );
    } else {
    ...
    <div id="searchInfo">
      ...
      <div className="detailsContainer">
        ...
        >
          ...
          <ul id="searchTrail">
            <li>
              &#9733; <AverageRating trail={result} /> &bull; {result?.length}mi
              &bull; {result?.difficulty}
            </li>
          </ul>
        </Link>
      </div>
