...
function TrailShow() {
  ...
  const reviews = useSelector(
    (state) => state?.trail?.[trail?.id]?.reviews || []
  );
  ...
  const favoriteTrailIds = Object.values(favorites)?.map(
    (favoriteObj) => favoriteObj?.favorite?.trail?.id
  );
  ...
  const favoriteForTrail = Object.values(favorites)?.find(
    (favoriteObj) => favoriteObj?.favorite?.trail?.id == trail?.id
  );
  ...
  const handleFavoriteClick = async () => {
    if (isFavorite) {
      await dispatch(
        removeFromFavorites(trail?.id, favoriteForTrail?.favorite?.id)
      );
    } else {
  ...
  if (!trail) {
    return (
      <div className="invalidTrail">
        ...
        <p id="invalidTrailStatment">
          The page you are looking for either does not exist or has a new link.
          Let us get you back on the right path.
        </p>
        <br />
        <button
          type="submit"
        ...
        <div className="showoutside">
          <form className="show">
            ...
            <div id="showtextimag">
              ...
              <span id="showtrailinfoimag">
                {trail?.difficulty} &bull;
                <span id="starshowimag">&#9733; </span>
                <AverageRating trail={trail} /> (
            ...
            <div id="modalAverage">
              <div id="averageRating">
                <FancyAverageRating trail={trail} />{" "}
              </div>
              <div id="smallMap">
                <SmallTrailMapWrapper trail={trail} />
              </div>
              <div id="reviewButton">
                {currentUser ? (
                  ...
                  <button
                    ...
                    id="reviewSignUp"
                  >
                    <p id="reviewSignUpContent">Sign Up to Write a Review</p>
                  </button>
                )}
              </div>
            </div>
                  ...
                  <div key={index} className="weatherCard">
                    <div id="weatherImag">
                      {day.weather[0].main === "Clear" && (
                        <img src={Sunny} alt="sun" id="weatherImagContent" />
                      )}
                      {day.weather[0].main === "Clouds" && (
                        <img src={Cloud} alt="cloud" id="weatherImagContent" />
                      )}
                      {day.weather[0].main === "Snow" && (
                        <img src={Snow} alt="cloud" id="weatherImagContent" />
                      )}
                      {day.weather[0].main === "Rain" && (
                        <img src={Rain} alt="cloud" id="weatherImagContent" />
                      )}
                      {day.weather[0].main === "Drizzle" && (
                        <img
                          src={Drizzle}
                          alt="cloud"
                          id="weatherImagContent"
                        />
                      )}
                    ...
                    <span className="high">
                      High: {Math.floor(day.temp.max)}&deg;F
                    </span>
            ...
            <div id="showHeaderContainer">
              <p
                id="reviewsHeaderShow"
                className={section === "R" ? "active" : ""}
                onClick={() => setSection("R")}
              >
                Reviews ({Object.values(trail?.reviews)?.length})
              </p>
              <p
                id="photoHeaderShow"
                className={section === "P" ? "active" : ""}
                onClick={() => setSection("P")}
              >
                Photos (
                {
                  photosArray?.filter((photo) => photo?.trail_id == trailId)
                    ?.length
                }
                )
              </p>
              ...
              <>
                ...
                <div id="PhotoButton">
                  <div>
                    ...
                    <p id="uploadPhotoContent1">
                      Photos help others preview the trail.{" "}
                    </p>
                    <p id="uploadPhotoContent">
                      {" "}
                      Upload photos about this trail to inspire others
                    </p>
                  </div>
                  {currentUser ? (
                    <>
                      <button
                        onClick={handleSettingUploadPhoto}
                        id="photoSignUp"
                      >
                        {" "}
                        <p id="photoSignUpContent1">Upload Photo</p>
                      </button>
                      {photoModalVisible && (
                        <PhotoUploadModal
                          trailId={trail?.id}
                          setVisible={setPhotoModalVisible}
                      ...
                      <>
                        <div>
                          <div
                            ...
                            id="showPhotoContainer"
                          >
                            <div id="showPhotoText">
                              <p id="clickToViewProfile">
                                Click to view profile
                              </p>
                              <p id="uploadedBy">
                                Uploaded by:
                                {capitalizeFirstLetter(photo?.user_fname)}{" "}
                                {capitalizeFirstLetter(photo?.user_lname)}{" "}
                              </p>
                              <p id="dateUploaded">
                                Date Uploaded:{formatDate(photo?.created_at)}
                              </p>
                            </div>
                            <img
                              src={photo?.image_url}
                              ...
                              onClick={() => {
                                window.scrollTo(0, 0);
                                navigate(`/profile/${photo?.user_id}`);
                              }}
                            />
                          </div>
                        </div>
