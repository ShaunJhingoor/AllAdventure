            ...
            <div className="carousel-buttons">
              {startIndex > 0 && (
                <button className="left-button" onClick={handleLeftClick}>
                  <img src={left} alt="left" id="rightarrowimg" />
                </button>
              )}
              {trails
                .slice(startIndex, startIndex + trailsPerPage)
                .map((trail, index) => (
                  <TrailsIndexItem
                    key={`${trail?.id}_${index}`}
                    trail={trail}
                    setCenter={setCenter}
                    setZoom={setZoom}
                  />
              ...
              {startIndex + trailsPerPage < trails.length && (
                <button className="right-button" onClick={handleRightClick}>
                  <img src={right} alt="right" id="rightarrowimg" />
                </button>
              )}
            </div>
          )}
